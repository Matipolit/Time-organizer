<script lang="ts">
    import { theme, PALETTES, MODES, type Palette, type ColorMode } from "../lib/stores/theme";
    import { Check, Trash, Monitor, Palette as PaletteIcon, Sun, Moon } from "lucide-svelte";
...
<div class="max-w-md mx-auto space-y-10 p-4">
    <section class="space-y-4">
        <h2 class="text-2xl font-bold flex items-center gap-2">
            <PaletteIcon size={24} /> Paleta kolorów
        </h2>
        <p class="text-muted-foreground">Wybierz główny zestaw kolorów aplikacji.</p>
    ...
    <section class="space-y-4">
        ...
        <p class="text-muted-foreground">Wymuś tryb jasny/ciemny lub synchronizuj z systemem.</p>
    ...
    <section class="pt-4 border-t border-muted">
        ...
        >
            ...
            <p class="text-sm text-foreground/80">To jest podgląd jak będą wyglądać kolory w wybranej konfiguracji.</p>

            <div class="flex gap-2">
                <Button variant="primary" class="text-xs py-1 h-8">Zrób to</Button>
                <Button variant="outline" class="text-xs py-1 h-8">Później</Button>
                <Button variant="destructive" class="text-xs py-1 h-8 px-2"><Trash size={14} /></Button>
            </div>
        </div>
    </section>
</div>
